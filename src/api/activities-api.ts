...
export const getAllActivities = async () => {
  try {
    const response = await springHttp.get<PagedResult<Activity>>('/activities');
...
export const getPagedActivities = async (page: number, size: number, search?: string, signal?: AbortSignal) => {
  try {
    const response = await springHttp.get<ActivitiesResponse>('/activities', {
      params: {
        page,
        size,
        search
...
export const getActivityById = async (id: string) => {
    const response = await springHttp.get<Activity>(`/activities/${id}`);
    return response.data;
};

export const createActivity = async (activityData: Omit<Activity, 'id' | 'createdDate' | 'lastUpdate'>) => {
    const response = await springHttp.post('/activities', activityData);
    return response.data;
};

export const updateActivity = async (id: string, activityData: Partial<Omit<Activity, 'id' | 'createdDate' | 'lastUpdate'>>) => {
    const response = await springHttp.patch(`/activities/${id}`, activityData);
    return response.data;
};

export const deleteActivity = async (id: string) => {
    const response = await springHttp.delete(`/activities/${id}`);
    return response.data;
};